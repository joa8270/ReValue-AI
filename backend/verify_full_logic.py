    ...
    try:
        ...
        if c:
            ...
            if c.get('element') == "Fire":
                print("   🔥 Element is CORRECTLY 'Fire' (Metal override removed).")
            else:
                print(f"   ⚠️ Element is '{c.get('element')}' (Expected Fire?)")
        else:
            print("   ⚠️ Citizen 10572 not found (might be random seed diff, but API generally works)")
    except Exception as e:
